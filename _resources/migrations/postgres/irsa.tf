resource "kubernetes_service_account_v1" "migrations_job_service_account" {
...
module "iam_role_for_postgres_service_account" {
  ...
  role_name = "${local.app_name}-${var.service_account_name}"

  oidc_providers = {
    main = {
      provider_arn = var.aws_iam_openid_connect_provider_arn
      namespace_service_accounts = [
        "${local.app_name}:${var.service_account_name}"
      ]
    }
  }
}
